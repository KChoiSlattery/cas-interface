...
function solve_equation(
  mathfield: any,
  output_span: HTMLSpanElement,
  copy_output_button: HTMLElement
) {
    ...
    success: function (data) {
      ...
      if (output["error_msg"] == "") {
        ...
        if (
          ...
          alert(
            "The selected variable is not in the equation. Please select another variable to solve for."
          );
        } else if (
          error_message ==
...
function fake_solve(
  ...
  katex.render("r=\\frac{2a\\mu }{av^2+\\mu }", output_span, {
    output: "mathml",
  });
  output_span.classList.remove("waiting");
  copy_output_button.classList.remove("hidden");
